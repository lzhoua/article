...
```
如果定义了toString()方法来进行类型转换的话，推荐显式调用toString()，因为内部的操作在尝试所有可能性之后，会尝试对象toString()方法尝试能否转化为String，所以直接调用这个方法效率会更高

#### 插入迭代器
如 `var name = values[i]; i++`； 前面两条语句可以写成 `var name = values[i++]`
